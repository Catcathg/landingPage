// Fonction pour rechercher un produit par SKU et renvoyer les données
function searchProductBySku(sku, showPrice) {
    var sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName('final');
    var data = sheet.getDataRange().getValues();

    for (var i = 1; i < data.length; i++) {  // Ignorer la première ligne (en-tête)
        if (data[i][0] == sku) {
            return {
                sku: data[i][0],
                nom: data[i][1],
                effets: data[i][2],
                poids: data[i][3],
                vertus: data[i][4],
                composition: data[i][5],
                utilisation: data[i][6],
                prix: showPrice ? data[i][7] : null,  // Condition pour afficher le prix
                codebarre: data[i][8],
                nomchinois: data[i][9],
                couleur: data[i][10],
                bio: data[i][11],
                brand: data[i][12],
                lot: data[i][13],
                icon: data[i][14],
            };
        }
    }
    return null;  // Retourne null si aucun produit n'est trouvé
}



// Fonction pour charger la page HTML
function doGet() {
    return HtmlService.createHtmlOutputFromFile('index')
        .setTitle('AB Label Generator | 小药包标签生成器')
        .setXFrameOptionsMode(HtmlService.XFrameOptionsMode.ALLOWALL);
}

